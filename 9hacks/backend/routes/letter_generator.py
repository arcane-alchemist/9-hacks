...
def generate_letter():
    ...
    Expected JSON body:
    {
        ...
        "language": "telugu",
        "important": true,           // optional: when true, use native language
        "native_language": "telugu"   // optional: used when important=true
    }
    ...
    For "important" letters, we automatically generate in the user's native
    language (native_language if provided, else language, else english).
    ...
    native_language = data.get("native_language", "").strip() or language

    # For important letters, prefer native language so user gets letter in their language
    if important and native_language:
