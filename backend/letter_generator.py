...
def generate_general_complaint(user_name: str, district: str, date: str, details: str, letter_type: str = "Complaint") -> str:
    """Fallback template for generalized legal or civil disputes (e.g. property dispute)"""
    return f"""To,
The Concerned Authority / Officer-in-Charge
District {district}

Date: {date}

Subject: Grievance/Complaint regarding {letter_type.replace('_', ' ').title()}

Respected Sir/Madam,

I, {user_name}, resident of District {district}, am writing this letter to bring to your attention a matter of serious concern regarding {letter_type.replace('_', ' ').title()}.

{details}

I request you to kindly take note of the above-mentioned circumstances and initiate appropriate action/inquiry as per the law. I am willing to provide any further information or documentation required to support my case.

Kindly acknowledge the receipt of this letter and take swift action to resolve this dispute/grievance to ensure justice is served.
...
def generate_letter(letter_type: str, user_name: str, district: str, date: str, details: str) -> str:
    ...
    if letter_type in templates:
        return templates[letter_type](user_name, district, date, details)
    else:
        return generate_general_complaint(user_name, district, date, details, letter_type)
