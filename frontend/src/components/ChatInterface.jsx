...
export default function ChatInterface({ situationType, onOpenLetterModal }) {
  const { t } = useT();
  const QUICK_PROMPTS = [
    { label: t('chat.prompts.p1') || "My employer hasn't paid my wages", category: 'Labour', icon: 'M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25m16.5 0a2.18 2.18 0 00.75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 00-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0112 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 01-.673-.38m0 0A2.18 2.18 0 013 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 013.413-.387m7.5 0V5.25A2.25 2.25 0 0013.5 3h-3a2.25 2.25 0 00-2.25 2.25v.894m7.5 0a48.667 48.667 0 00-7.5 0M12 12.75h.008v.008H12v-.008z' },
    { label: t('chat.prompts.p2') || 'I need protection from domestic abuse', category: 'Safety', icon: 'M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z' },
    { label: t('chat.prompts.p3') || 'My land is being taken illegally', category: 'Property', icon: 'M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25' },
    { label: t('chat.prompts.p4') || 'I faced caste discrimination', category: 'SC/ST Rights', icon: 'M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18' },
    { label: t('chat.prompts.p5') || 'My employer is forcing overtime without pay', category: 'Labour', icon: 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z' },
    { label: t('chat.prompts.p6') || 'I want to file an RTI application', category: 'RTI', icon: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z' },
  ];
  ...
  async function sendQuery(text, extraContext) {
    const fullText = extraContext ? extraContext : text;
    setMessages((prev) => [...prev, { role: 'user', text: fullText }]);
    setLoading(true);
    try {
      const history = messages.map(m => ({ role: m.role === 'user' ? 'user' : 'assistant', content: m.text || m.question || JSON.stringify(m.data) }));
      const data = await postQuery({ text: fullText, situation_type: situationType || undefined, history });
      if (data.clarification_needed) {
        pendingClarificationRef.current = text;
  ...
  async function handlePincodeSubmit(e) {
    ...
    try {
      ...
      const finalDomain = lastAiMsg?.data?.domain || situationType || 'general';
      const dlsa = await getDLSA(pincode.trim(), finalDomain);
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white leading-none">LegalSaathi</p>
        ...
        <div className="min-h-[300px] max-h-[60vh] overflow-y-auto px-5 py-6 space-y-5 chat-scroll">
          ...
          {isEmpty && (
            <div className="flex flex-col items-center justify-center py-4 text-center">
              ...
              <h3 className="text-base font-semibold text-white mb-1 tracking-tight">{t('chat.greeting') || 'How can I help you today?'}</h3>
                ...
                {QUICK_PROMPTS.map((prompt) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className={`text-[10px] mt-0.5 font-medium ${(categoryColors[prompt.category] || 'text-teal-400 bg-teal-500/10 border-teal-500/20').split(' ')[0]}`}>{t(`chat.cats.${prompt.category}`) || prompt.category}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            if (msg.role === 'user') return (
              <div key={i} className="flex justify-end msg-enter">
                <div className="max-w-xs sm:max-w-sm">
                  <div className="bg-teal-600/80 text-white rounded-2xl rounded-br-sm px-4 py-3 text-sm leading-relaxed border border-teal-500/20">
                    {msg.text}
                  </div>
...
function DLSACard({ data }) {
  ...
  const googleMaps = office?.google_maps_search_term || '';
  const requirements = office?.what_to_take_with_you || '';

  return (
    <div className="max-w-lg bg-teal-500/10 border border-teal-500/20 rounded-2xl rounded-tl-sm p-4 space-y-4">
      <div className="space-y-2">
        ...
        {googleMaps && (
           <p className="text-xs text-gray-400">Search: <strong className="text-white">{googleMaps}</strong></p>
        )}
        {requirements && (
           <div className="mt-2 bg-black/20 p-2 rounded-lg text-xs text-gray-300">
             <span className="font-semibold text-teal-300/80 uppercase mb-1 block">What to take:</span>
             {requirements}
           </div>
        )}
      </div>
      ...
      {data?.dlsa_office && (
        <div className="pt-3 border-t border-teal-500/20 mt-3 space-y-2">
           <h4 className="text-xs font-semibold text-teal-200/70 flex items-center gap-1.5">
             ...
             <span className="ml-auto text-[9px] font-bold uppercase tracking-wider bg-green-500/15 text-green-400 border border-green-500/20 px-1.5 rounded-full">
                {t('response.free')}
             </span>
           </h4>
           <div className="text-sm">
             <p className="font-medium text-teal-100">{data.dlsa_office.name}</p>
             <p className="text-gray-300 text-xs mt-0.5">{data.dlsa_office.address}</p>
             {data.dlsa_office.phone && <p className="text-xs text-teal-300 mt-1">&#9990; {data.dlsa_office.phone}</p>}
           </div>
        </div>
      )}
    </div>
